...
Object.keys(r).forEach((k) => {
  ...
  Object.values(r[k].saves).forEach(
    (s) =>
      (leaderboard[k][s.user.id] = { xp: s.xp || 0, id: s.id, name: s.name }),
  );
});
const maps = {};
...
io.on("connection", (socket) => {
  ...
  socket.on("login", (data) => {
    ...
    const p = rooms[user.room].saves[user.id]
      ? rooms[user.room].saves[user.id]
      : rooms[user.room].defaultMap
        ? defaults.player(socket.id, 160, 80, 7)
        : defaults.player(socket.id);
    ...
    if (rooms[user.room].teamMap)
      p.team = k[Math.floor(Math.random() * k.length)];
    players[user.room][socket.id] = p;
    db.set({ rooms });
    socket.broadcast
      .to(user.room)
      .emit("update player", players[user.room][socket.id]);
    socket.emit("user", p);
  });
  ...
  socket.on("logout", () => {
    ...
    players[user.room][socket.id] = rooms[user.room].defaultMap
      ? defaults.player(socket.id, 160, 80, 7)
      : defaults.player(socket.id);
    const k = rooms[user.room].teams;
    if (rooms[user.room].teamMap)
  ...
  socket.on("give item", ([id, item, amount]) =>
    id &&
    players[user.room][user.id] &&
    (devs[players[user.room][user.id].user?.name] ||
      db.get("rooms")[user.room].admins[players[user.room][user.id].user?.id])
      ? socket.to(id).emit("give item", [item, amount])
      : "",
  );
  socket.on("tp", ([id, x, y, scene]) =>
    id &&
    players[user.room][user.id] &&
    (devs[players[user.room][user.id].user?.name] ||
      db.get("rooms")[user.room].admins[players[user.room][user.id].user?.id])
      ? socket.to(id).emit("tp", [x, y, scene])
      : "",
  );
  socket.on("clear inventory", (id) =>
    id &&
    players[user.room][user.id] &&
    (devs[players[user.room][user.id].user?.name] ||
      db.get("rooms")[user.room].admins[players[user.room][user.id].user?.id])
      ? socket.to(id).emit("clear inventory")
      : "",
  );
  socket.on("clear backpack", (id) =>
    id &&
    players[user.room][user.id] &&
    (devs[players[user.room][user.id].user?.name] ||
      db.get("rooms")[user.room].admins[players[user.room][user.id].user?.id])
      ? socket.to(id).emit("clear backpack")
      : "",
  );
  socket.on("kill", (id) =>
    id &&
  ...
  socket.on("ban", (id) => {
    ...
    if (rooms[user.room].admins[id] || devs[players[user.room][id].user?.name])
      return;
  ...
  socket.on("op", (id) => {
    ...
    const rooms = db.get("rooms") || {};
    if (
      !rooms[user.room].admins[user.id] &&
      !devs[user.name] &&
      user.name != rooms[user.room].creator
    )
      return;
  ...
  socket.on("unop", (id) => {
    ...
    if (!p?.user) return;
    if (
      !rooms[user.room].admins[p.user.id] ||
      p.user.name == rooms[user.room].creator
    )
      return;
  ...
  socket.on("update map", (d) => {
    ...
    if (
      players[user.room][socket.id].editor &&
      !devs[players[user.room][socket.id].user?.name]
    )
      return;
  ...
  socket.on("update player", (changes) => {
    ...
    Object.keys(changes).forEach((k) => {
      p[k] = changes[k];
      if (k == "xp" && p.user) {
        leaderboard[user.room][p.user.id] = {
          name: p.name,
          xp: p.xp,
          id: p.user.id,
          team: p.team,
    ...
    socket.broadcast
      .to(user.room)
      .emit("update player", { ...changes, id: socket.id });
  });
  ...
  socket.on("join room", (id) => {
    ...
    user.room = id;
    p = rooms[user.room].saves[user.id]
      ? rooms[user.room].saves[user.id]
      : rooms[user.room].defaultMap
        ? defaults.player(socket.id, 160, 80, 7)
        : defaults.player(socket.id);
  ...
  socket.on("delete room", (roomId, cb) => {
    ...
    if (
      !rooms[roomId] ||
      (rooms[roomId].creator != user.name && !devs[user.name])
    )
      return;
    const ban = user ? user.id : "";
  ...
  socket.on(
    ...
    ({
      ...
      rooms[roomId] = {
        ...
        admins: { [user.id]: true },
        leaderboard: {
          [user.id]: { xp: 0, id: user.id, name: user.name, team: p.team },
        },
        teams: [],
        saves: {},
        banned: [],
      ...
      if (teamMap) {
        for (let i = 0; i < numTeams; i++)
          rooms[roomId].teams[i] = teamColors[i];
      }
      db.set({ rooms });
      if (user.room) socket.leave(user.room);
      user.room = roomId;
  ...
  socket.on("get rooms", (cb) => {
    ...
    Object.keys(rooms).forEach((k) => {
      ...
      if (devs[user.name] || public)
        r[k] = {
          name,
          id,
          public,
          creator,
          online: Object.keys(players[k]).length,
          banned,
          teamMap,
          size: getSize(rooms[k]),
        };
    });
    cb(r);
  });
...
const updateEntities = () => {
  ...
  Object.keys(rooms).forEach((k) => {
    ...
    entities[k].forEach((sc, s) => {
      ...
      if (
        sc.length == 0 ||
        !Object.values(players[k]).some((p) => p.scene == s)
      )
        return;
      Object.values(entities[k][s])
        .filter((e) => e?.enemy)
        .forEach((e, i) => {
          const changes = JSON.stringify(e),
            eChanges = {};
          ...
          Object.keys(e).forEach((k) =>
            JSON.parse(changes)[k] != e[k] && k != "i" && k != "frame"
              ? (eChanges[k] = e[k])
              : "",
          );
          if (
            (kill != i || kill === false) &&
            Object.keys(eChanges).length != 0
          )
            io.to(k).emit("update entity", [eChanges, i, s]);
        });
      Object.values(entities[k][s])
        .filter((e) => e?.animal)
        .forEach((e, i) => {
...
const gameLoop = () => {
  ...
  Object.keys(rooms).forEach((k) => {
    ...
    Object.values(players[k]).forEach((p) =>
      p.id != "offline" && p.bed ? sleeping++ : "",
    );
    r.daylight++;
    if (
      r.daylight >= 24 ||
      (sleeping >= Math.ceil(Object.keys(players[k]).length / 2) &&
        sleeping > 0)
    )
      r.daylight = 0;
    ...
    r.map.forEach((m, scene) => {
      if (m.type != "house") {
        ...
        for (let i = 0; i < maxEntities; i++) {
          if (r.daylight >= 15)
            enemies.spawn({ scene }, maps[k], entities[k][scene]);
          else if (m.type != "cave")
            animals.spawn({ scene }, maps[k], entities[k][scene]);
        }
      }
    });
    io.to(k).emit("init entities", entities[k]);
    Object.keys(entities[k]).forEach(
      (e, i) => (r.map[i].entities = entities[k][i]),
    );
    if (!r.leaderboard) r.leaderboard = {};
    Object.keys(leaderboard[k]).forEach(
      (e, i) => (r.leaderboard[e] = leaderboard[k][e]),
    );
    r.map.forEach((m) => {
      ...
      m.entities.forEach((e) => {
        let moving = false;
        Object.values(players[k]).forEach((p) =>
          p.minecart == m.entities.indexOf(e) ? (moving = true) : "",
        );
        if (!moving && e.id != 2) {
          e.x = e.dx;
          e.y = e.dy;
          e.moving = false;
          io.to(k).emit("update entity", [
            e,
            m.entities.indexOf(e),
            r.map.indexOf(m),
          ]);
        }
        if ((e.type == "arrow" && !players[k][e.from]) || !e.x || !e.y) {
          io.to(k).emit("remove entity", [
            m.entities.indexOf(e),
            r.map.indexOf(m),
          ]);
